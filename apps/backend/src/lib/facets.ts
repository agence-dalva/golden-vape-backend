...
/*
  Machinerie commune aux facettes.

  Deux pages du site filtrent le même catalogue selon les mêmes critères : une rubrique et une
  marque. Seul change l'ensemble de départ — les produits d'une catégorie et de sa descendance
  d'un côté, ceux qui portent une marque de l'autre. Tout le reste, le comptage par valeur, la
...
/**
 * Facettes et page d'identifiants pour un ensemble de départ quelconque.
 *
 * `cible` doit être un fragment de CTE nommé `cible` rendant `id`, `title` et `created_at`,
 * ses paramètres passés dans `bindings` — ils viennent en premier dans chaque requête.
 */
export async function repondreFacettes(
  knex: Knex,
  {
    cible,
    bindings,
  ...
  */
  ...
  const pageQuery = knex.raw(
    `WITH ${cible},${VALEURS}
     ...
     ORDER BY ${orderBy}
     LIMIT ? OFFSET ?`,
    [...bindings, ...retenus.bindings, take, skip]
  )
  ...
  const facets = new Map<
    string,
    { type: string; allow_multiple: boolean; values: { value: string; count: number }[] }
  >()
  for (const ligne of comptes) {
    const facette =
      facets.get(ligne.type) ?? { type: ligne.type, allow_multiple: ligne.allow_multiple, values: [] }

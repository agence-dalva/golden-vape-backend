...
export class Migration20260701063005 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table if not exists "attribute_type" ("id" text not null, "name" text not null, "preset_values" jsonb not null default '[]', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "attribute_type_pkey" primary key ("id"));`);
